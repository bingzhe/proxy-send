<template>
  <sl-dialog ref="uploadGoodsExecl" :title="title" :showFooter="false" :showClose="false">
...
export default {
  ...
  methods: {
    ...
    async starHandle() {
      ...
      console.log('创建后台任务 req=>', data)
      const resp = await taskSave(data)
      console.log('创建后台任务 res=>', resp)
    ...
    async stopHandleFile() {
      ...
      const resp = await taskSave(data)
      console.log('取消后台任务 res=>', JSON.stringify(resp.data))
