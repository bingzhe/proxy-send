<template>
  <sl-dialog ref="uploadGoodsExecl" title="检查商品" :showFooter="false" :showClose="false">
...
import { progressGet, goodsGet } from '@/api/api'
...
export default {
  ...
  methods: {
    ...
    async starHandle() {
      if (!this.file_id) {
        return this.$message.error('请先上传检查的文件')
      }
      const data = {
        opr: 'goods_check_from_file',
        file_id: this.file_id, // 文件上传接口返回的文件id
        file_md5: this.file_md5 // 文件上传接口返回的文件md5
      }

      console.log('开始检查， req=>', data)
      const resp = await goodsGet(data)
      console.log('开始检查， res=>', resp)
